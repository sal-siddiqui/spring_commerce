...
public abstract class BaseServiceImplementer {
    ...
    public <T> Boolean checkResourceExists(final JpaRepository<T, Long> repository, final Long id) {
        return repository.existsById(id);
    }


    // Retrieves a resource by ID or throws a ResourceNotFoundException if not found
    public <T> T getOrThrow(final JpaRepository<T, Long> repository, final Long id, final String resourceName) {
        return repository.findById(id).orElseThrow(() -> new ResourceNotFoundException(resourceName, "ID", id));
    }


    // Builds a Pageable object with validation for page, size, sort field, and
    // order
    public <T> Pageable getPageable(final Class<T> entityClass, final Integer pageNumber, final Integer pageSize,
        ...
        final boolean validSortField =
                Arrays.stream(entityClass.getDeclaredFields()).anyMatch(field -> field.getName().equals(sortBy));
    ...
    // invalid
    public <T> Page<T> getPaginatedResponse(final JpaRepository<T, Long> repository, final Class<T> entityClass,
            final Integer pageNumber, final Integer pageSize, final String sortBy, final String sortOrder) {

        final Pageable pageable = this.getPageable(entityClass, pageNumber, pageSize, sortBy, sortOrder);
